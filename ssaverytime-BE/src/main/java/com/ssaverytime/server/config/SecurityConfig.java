...
public class SecurityConfig {
	...
	public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
		http
				...
				.formLogin(form -> form.loginProcessingUrl("/api/v1/auth/login") // POST 요청 시 로그인
						.successHandler(customAuthenticationSuccessHandler).permitAll())

				.logout(logout -> logout.logoutUrl("/api/v1/logout") // POST 요청 시 로그아웃
				...
				.exceptionHandling(ex -> ex.authenticationEntryPoint(
						(req, res, authException) -> res.sendError(HttpServletResponse.SC_UNAUTHORIZED, "로그인이 필요합니다.")))
	...
	public CorsConfigurationSource corsConfigurationSource() {
		...
		// 개발 환경 편의를 위해 모든 출처 허용 (배포 시 보안을 위해 구체적인 도메인으로 제한 필요)
